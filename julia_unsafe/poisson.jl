...
#
# Same Jacobi stencil as `julia/poisson.jl`. The interior loop uses
# `@inbounds` so index checks are skipped (i±1 and j±1 stay inside
# the array because the loops only visit interior points). `@fastmath`
# lets LLVM reorder FP ops and vectorize the stencil. Buffers are
# swapped instead of copied; both stay zero on the Dirichlet boundary
# because only interior points are written.
# ------------------------------------------------------------
