...
///
/// `u`, `u_new`, and `rhs` are raw pointers into length-`N * N` column-major
/// buffers. `ptr.add(k)` is pointer arithmetic (`&ptr[k]` in C), not a
/// numeric add; `.read()` / `.write()` load and store that element without
/// bounds checks. Layout: index `k = i + N * j`, so `k ± 1` is a neighbor
/// in `i` and `k ± N` is a neighbor in `j`.
///
/// # Safety
/// `u`, `u_new`, and `rhs` must each have length `N * N`.
    ...
    for i in 1..N - 1 {
        ...
        let val = 0.25
            * (u.add(k + 1).read()
                ...
                + u.add(k - N).read()
                + h2 * rhs.add(k).read());
        local = local.max((val - u.add(k).read()).abs());
        u_new.add(k).write(val);
    }
    local
}
...
fn jacobi<const N: usize>(
    ...
    debug_assert_eq!(u_new.len(), N * N);
    debug_assert_eq!(rhs.len(), N * N);

    for iter in 1..=maxiter {
        let u_ptr = u.as_ptr();
        let u_new_ptr = u_new.as_mut_ptr();
        let rhs_ptr = rhs.as_ptr();

        update_error = 0.0;
        for j in 1..N - 1 {
            // Safety: j is an interior column; buffers are N×N.
            update_error = update_error
                .max(unsafe { jacobi_column::<N>(u_ptr, u_new_ptr, rhs_ptr, j, h2) });
        }
